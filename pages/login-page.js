...
export class LoginPage {
    ...
    async checkLoginPageExist(){
      const loginContainer = await helper.getLocator(this.page, this.loginContainer);
      await expect(loginContainer).toBeVisible();
      const loginForm = await helper.getLocator(this.page, this.form);
      await expect(loginForm).toBeVisible();
      const userNamePlaceholder = await helper.getLocator(this.page,this.userNamePlaceholder);
      await expect(userNamePlaceholder).toBeVisible();
      const passwordPlaceholder = await helper.getLocator(this.page,this.passwordPlaceholder);
